...
const PracticeMayura: React.FC = () => {
  ...
  const [feedback, setFeedback] = useState([
    ...
    { type: 'warning', message: 'Extend thumb and little finger', timestamp: '0:32' },
    { type: 'correct', message: 'Excellent ring finger bend', timestamp: '0:45' }
  ]);
  ...
  const mudraInstructions = {
    ...
    steps: [
      'Extend thumb and little finger straight',
      'Bend index, middle, and ring fingers into palm',
      'Keep extended fingers curved like peacock beak',
      'Palm facing outward'
    ]
  };
  ...
  const calculateMudraAccuracy = (landmarks: { x: number; y: number; z: number }[]) => {
    ...
    // Mayura Mudra Analysis
    let accuracy = 0;
    let totalChecks = 0;

    // 1. Check thumb and little finger extended (25 points)
    const extendedFingersScore = checkExtendedFingers(landmarks);
    accuracy += extendedFingersScore;
    totalChecks += 25;

    // 2. Check index, middle, ring fingers bent into palm (25 points)
    const bentFingersScore = checkBentFingers(landmarks);
    accuracy += bentFingersScore;
    totalChecks += 25;

    // 3. Check extended fingers curved (15 points)
    ...
    // 5. Check finger spacing (15 points)
  ...
  const checkExtendedFingers = (landmarks: { x: number; y: number; z: number }[]) => {
    // Check thumb and little finger are extended
    const extendedTips = [4, 20]; // thumb, little tips
    const extendedPIPs = [3, 18]; // corresponding PIP joints
  ...
  const checkBentFingers = (landmarks: { x: number; y: number; z: number }[]) => {
    // Check index, middle, ring fingers are bent into palm
    const fingerTips = [8, 12, 16]; // index, middle, ring tips
    const fingerMCPs = [5, 9, 13]; // corresponding MCP joints
    ...
    fingerTips.forEach((tipIndex, fingerIndex) => {
      ...
      // Check if tip is below MCP (bent into palm)
      if (tip.y > mcp.y) {
        bentCount++;
      }
    });
  ...
  const checkFingerCurve = (landmarks: { x: number; y: number; z: number }[]) => {
    // Check extended fingers are curved like peacock beak
    const thumbTip = landmarks[4];
    const littleTip = landmarks[20];
    const thumbIP = landmarks[3];
    const littleDIP = landmarks[19];

    // Calculate curve by checking if tips are not in straight line with intermediate joints
  ...
  const checkFingerSpacing = (landmarks: { x: number; y: number; z: number }[]) => {
    // Check spacing between extended fingers
    const thumbTip = landmarks[4];
    const littleTip = landmarks[20];
    ...
    // For Mayura, extended fingers should be appropriately spaced
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        >
          <h1 className="text-4xl md:text-5xl font-serif font-bold text-[#1F4E79] mb-4">
            Practice Mayura Mudra
          </h1>
          <p className="text-xl text-gray-600">
            Real-time AI feedback for perfect Mayura formation
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                          ...
                          <div className="mt-2">
                            <span className="text-xs text-white">
                              {accuracy >= 90
                                ? 'Excellent Mayura!'
                                : accuracy >= 75
                                ? 'Good Mayura, keep practicing!'
                                : 'Mayura needs improvement'}
                            </span>
                          </div>
                        )}
                      </div>
          ...
          >
            ...
            <GlassCard className="p-6 bg-white/80">
              <AIGuru
                message="Extend your thumb and little finger while keeping the others bent into your palm."
