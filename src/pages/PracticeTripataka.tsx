...
const PracticeTripataka: React.FC = () => {
  ...
  const [feedback, setFeedback] = useState([
    ...
    { type: 'warning', message: 'Ring finger needs to be bent', timestamp: '0:32' },
  ...
  const mudraInstructions = {
    name: 'Tripataka',
    sanskrit: 'त्रिपताका',
    description: 'Three fingers extended, ring finger bent at second joint',
    steps: [
      'Extend thumb, index, and middle fingers straight',
      'Bend ring finger at the second joint',
      'Keep little finger slightly bent or extended',
      'Palm facing outward'
    ]
  };
  ...
  const checkExtendedFingers = (landmarks: { x: number; y: number; z: number }[]) => {
    // Check thumb, index, middle fingers are extended
    const extendedTips = [4, 8, 12]; // thumb, index, middle tips
    const extendedPIPs = [3, 6, 10]; // corresponding PIP joints
  ...
  const checkRingFingerBend = (landmarks: { x: number; y: number; z: number }[]) => {
    // Check ring finger is bent at second joint (PIP)
    ...
    // For Tripataka, ring finger should be bent (angle > 90 degrees)
    if (angle > 90) {
      return 25;
    } else if (angle > 60) {
      return 15;
  ...
  const checkLittleFingerPosition = (landmarks: { x: number; y: number; z: number }[]) => {
    // Little finger can be slightly bent or extended
    ...
    // Little finger should be somewhat extended or slightly bent
    if (angle < 45) {
      return 15;
    } else if (angle < 60) {
      return 10;
  ...
  const checkFingerSpacing = (landmarks: { x: number; y: number; z: number }[]) => {
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        >
          <h1 className="text-4xl md:text-5xl font-serif font-bold text-[#1F4E79] mb-4">
            Practice Tripataka Mudra
          </h1>
          <p className="text-xl text-gray-600">
            Real-time AI feedback for perfect Tripataka formation
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                          ...
                          <div className="mt-2">
                            <span className="text-xs text-white">
                              {accuracy >= 90
                                ? 'Excellent Tripataka!'
                                : accuracy >= 75
                                ? 'Good Tripataka, keep practicing!'
                                : 'Tripataka needs improvement'}
                            </span>
                          </div>
                        )}
                      </div>
          ...
          >
            ...
            <GlassCard className="p-6 bg-white/80">
              <AIGuru
                message="Focus on bending your ring finger at the second joint while keeping the others extended."
...
export default PracticeTripataka;
