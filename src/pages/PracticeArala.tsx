...
const PracticeArala: React.FC = () => {
  ...
  const [feedback, setFeedback] = useState([
    ...
    { type: 'warning', message: 'Bend your little finger more', timestamp: '0:32' },
    { type: 'correct', message: 'Excellent other fingers', timestamp: '0:45' }
  ]);
  ...
  const mudraInstructions = {
    ...
    steps: [
      'Extend thumb, index, middle, and ring fingers straight',
      'Bend little finger into palm',
      'Keep extended fingers close together',
      'Palm facing outward'
    ]
  };
  ...
  const calculateMudraAccuracy = (landmarks: { x: number; y: number; z: number }[]) => {
    ...
    // Arala Mudra Analysis
    let accuracy = 0;
    let totalChecks = 0;

    // 1. Check thumb, index, middle, ring fingers extended (25 points)
    const extendedFingersScore = checkExtendedFingers(landmarks);
    accuracy += extendedFingersScore;
    totalChecks += 25;

    // 2. Check little finger bent into palm (25 points)
    const bentFingerScore = checkBentFinger(landmarks);
    accuracy += bentFingerScore;
    totalChecks += 25;

    // 3. Check extended fingers close together (15 points)
    ...
    // 5. Check overall hand alignment (15 points)
    const handAlignmentScore = checkHandAlignment(landmarks);
    accuracy += handAlignmentScore;
    totalChecks += 15;
  ...
  const checkExtendedFingers = (landmarks: { x: number; y: number; z: number }[]) => {
    // Check thumb, index, middle, ring fingers are extended
    const extendedTips = [4, 8, 12, 16]; // thumb, index, middle, ring tips
    const extendedPIPs = [3, 6, 10, 14]; // corresponding PIP joints
    ...
    return (extendedCount / 4) * 25;
  };

  const checkBentFinger = (landmarks: { x: number; y: number; z: number }[]) => {
    // Check little finger is bent into palm
    ...
    if (littleTip.y > littleMCP.y) {
      return 25;
    } else if (littleTip.y > littleMCP.y - 0.05) {
      return 15;
    } else {
      return 0;
    }
  ...
  const checkFingerProximity = (landmarks: { x: number; y: number; z: number }[]) => {
    // Check extended fingers are close together (excluding little finger)
    const extendedTips = [4, 8, 12, 16];
    const xPositions = extendedTips.map(index => landmarks[index].x);
    ...
    const proximityScore = Math.max(0, 15 - (avgSpacing * 300));
  ...
  const checkHandAlignment = (landmarks: { x: number; y: number; z: number }[]) => {
    // Check overall alignment of extended fingers
    const extendedTips = [4, 8, 12, 16];
    const yPositions = extendedTips.map(index => landmarks[index].y);
    ...
    const alignmentScore = Math.max(0, 15 - (variance * 2000));
  ...
  const generateMudraFeedback = (landmarks: { x: number; y: number; z: number }[], accuracy: number) => {
    ...
    if (extendedFingersScore < 20) {
      newFeedback.push({
        type: 'warning',
        message: 'Extend your thumb, index, middle, and ring fingers straight',
        timestamp: currentTime
      });
    } else if (extendedFingersScore >= 20) {
      newFeedback.push({
        type: 'correct',
        message: 'Excellent extension of the main fingers!',
        timestamp: currentTime
      });
    }

    if (bentFingerScore < 15) {
      newFeedback.push({
        type: 'warning',
        message: 'Bend your little finger into your palm',
        timestamp: currentTime
      });
    } else if (bentFingerScore >= 15) {
      newFeedback.push({
        type: 'correct',
        message: 'Perfect little finger bend!',
        timestamp: currentTime
      });
    }

    if (fingerProximityScore < 10) {
      newFeedback.push({
        type: 'warning',
        message: 'Keep your extended fingers close together',
        timestamp: currentTime
      });
    }
    ...
    if (handAlignmentScore < 10) {
      newFeedback.push({
        type: 'warning',
        message: 'Align your extended fingers in a straight line',
        timestamp: currentTime
      });
    }
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        >
          <h1 className="text-4xl md:text-5xl font-serif font-bold text-[#1F4E79] mb-4">
            Practice Arala Mudra
          </h1>
          <p className="text-xl text-gray-600">
            Real-time AI feedback for perfect Arala formation
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                          ...
                          <div className="mt-2">
                            <span className="text-xs text-white">
                              {accuracy >= 90
                                ? 'Excellent Arala!'
                                : accuracy >= 75
                                ? 'Good Arala, keep practicing!'
                                : 'Arala needs improvement'}
                            </span>
                          </div>
                        )}
                      </div>
          ...
          >
            ...
            <GlassCard className="p-6 bg-white/80">
              <AIGuru
                message="Keep your thumb, index, middle, and ring fingers extended while bending only the little finger."
